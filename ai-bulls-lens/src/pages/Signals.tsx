...
export default function Signals() {
  ...
  const { data: signals, loading } = useApi(() => api.signals.opportunities(), []);
            ...
            : (signals ?? []).map((signal, i) => (
                <motion.div
                  ...
                  className={`bg-card border rounded-xl p-5 cursor-pointer hover:shadow-lg transition-all ${
                    signal.type === "Bullish"
                      ? "border-bullish/20 hover:border-bullish/40"
                      : "border-bearish/20 hover:border-bearish/40"
                  }`}
                >
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      <div
                        className={`h-10 w-10 rounded-lg flex items-center justify-center font-heading font-bold text-sm ${
                    ...
                    <div className="h-2 bg-secondary rounded-full overflow-hidden">
                      <motion.div
                        ...
                        className={`h-full rounded-full ${
                          signal.confidence >= 85 ? "bg-bullish" : signal.confidence >= 70 ? "bg-warning" : "bg-bearish"
                        }`}
                      />
                    </div>
                  </div>
