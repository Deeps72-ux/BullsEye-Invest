...
import { TrendingUp, TrendingDown, DollarSign, BarChart3, Radar, ArrowUpRight, ArrowDownRight } from "lucide-react";
import { mockStocks, mockSignals, mockPortfolio, mockChartData } from "@/lib/mock-data";
...
export default function Dashboard() {
  ...
  const topMovers = [...mockStocks].sort((a, b) => Math.abs(b.changePercent) - Math.abs(a.changePercent)).slice(0, 4);
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: "Portfolio Value", value: `$${mockPortfolio.totalValue.toLocaleString()}`, icon: DollarSign, change: mockPortfolio.dayChangePercent, sub: `$${mockPortfolio.dayChange.toFixed(0)} today` },
            { label: "Total Returns", value: `$${mockPortfolio.totalPnl.toLocaleString()}`, icon: TrendingUp, change: mockPortfolio.totalPnlPercent, sub: "All time" },
            { label: "Top Signals", value: mockSignals.filter(s => s.confidence > 85).length.toString(), icon: Radar, sub: "High confidence" },
            { label: "Active Stocks", value: mockStocks.length.toString(), icon: BarChart3, sub: "Being tracked" },
          ].map((card, i) => (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          >
            <h2 className="font-heading font-semibold text-foreground mb-4">Portfolio Performance</h2>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={mockChartData}>
                  ...
                  <Tooltip
                    contentStyle={{ background: "hsl(220, 18%, 10%)", border: "1px solid hsl(220, 15%, 18%)", borderRadius: "8px", color: "hsl(210, 20%, 95%)" }}
                  />
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => navigate("/signals")} className="text-xs text-primary hover:underline">View all</button>
            </div>
            <div className="space-y-3">
              {mockSignals.slice(0, 4).map((signal) => (
                <div key={signal.id} className="flex items-center justify-between p-3 rounded-lg bg-secondary/50 hover:bg-secondary transition-colors cursor-pointer"
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => navigate("/markets")} className="text-xs text-primary hover:underline">View all</button>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {topMovers.map((stock) => (
              <div key={stock.symbol} className="p-4 rounded-lg bg-secondary/50 hover:bg-secondary transition-colors cursor-pointer"
